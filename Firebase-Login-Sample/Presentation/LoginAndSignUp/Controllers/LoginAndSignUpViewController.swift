...
final class LoginAndSignUpViewController: UIViewController {
    ...
    enum AuthViewType {
        ...
        var segueID: String {
            switch self {
                case .login: return "loginSegueId"
                case .signUp: return "signUpSegueId"
            }
        }
    }
    private func segueType(_ segueID: String) -> AuthViewType {
        if AuthViewType.login.segueID == segueID {
            return .login
        }
        return .signUp
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        switch segueType(segue.identifier ?? "") {
            case .login:
